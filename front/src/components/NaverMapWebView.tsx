...
<body>
  <div id="map"></div>
  <script>
    var map, markers = [], placeMarkers = [], userMarker = null, selectedId = null, selectedPlaceIndex = null;
    var restaurantData = [], placeData = [];
    ...
    function markerHtml(r, selected){
      ...
      var scale = selected ? 'scale(1.2)' : 'scale(1)';
      var ring = selected ? '0 0 0 4px rgba(255,255,255,.95),0 3px 10px rgba(0,0,0,.45)' : '0 1px 3px rgba(0,0,0,.4)';
      return '<div style="transform:translate(-50%,-100%) '+scale+';transform-origin:bottom center;display:flex;flex-direction:column;align-items:center;transition:transform .12s ease;">'
        + '<div style="background:'+bg+';color:#fff;font-weight:700;font-size:12px;padding:5px 9px;border-radius:14px;white-space:nowrap;box-shadow:'+ring+';">'
        + (selected ? '✓ 선택 · ' : '') + '👍 ' + r.count + '</div>'
    ...
    function renderPlaces(list){
      ...
      list.forEach(function(p, index){
        var m = new naver.maps.Marker({
          ...
          map: map,
          title: p.name,
          icon: { content: (function(){
            var selected = index === selectedPlaceIndex;
            var bg = selected ? '${colors.PINK_700}' : '#fff';
            var color = selected ? '#fff' : '#333';
            var border = selected ? '${colors.RED_500}' : '${colors.PINK_700}';
            var scale = selected ? 'scale(1.14)' : 'scale(1)';
            var shadow = selected ? '0 0 0 4px rgba(255,255,255,.95),0 4px 12px rgba(0,0,0,.45)' : '0 1px 4px rgba(0,0,0,.35)';
            return '<div style="transform:translate(-50%,-100%) '+scale+';transform-origin:bottom center;background:'+bg+';border:3px solid '+border+';border-radius:18px;padding:6px 9px;font-size:12px;font-weight:800;color:'+color+';white-space:nowrap;box-shadow:'+shadow+';transition:transform .12s ease;">'
              + (selected ? '✓ 선택 · ' : '🍽 ') + p.name + '</div>';
        ...
        naver.maps.Event.addListener(m, 'click', function(){
          ...
          renderMarkers(restaurantData);
          renderPlaces(placeData);
          post({type:'placeClick', index:index});
        });
        placeMarkers.push(m);
      });
    }
    ...
    window.setData = function(list, places, sel){
      selectedId = sel;
      if (sel !== null) selectedPlaceIndex = null;
      renderMarkers(list);
      renderPlaces(places);
    };
